...
from comms.schema import (
    AnnouncementType,
    ParentNotificationSettingsType,
    InstructorNotificationSettingsType,
    SessesionNotificationSettingsType
)
...
class CreateAnnouncement(graphene.Mutation):
    ...
    def mutate(root, info, **validated_data):
        ...
        announcement, created = Announcement.objects.update_or_create(
            id=validated_data.pop("announcement_id", None), defaults=validated_data
        )
        if should_email:
            ...
            for enrollment in course.enrollment_set.all():
                ...
                primary_parent_fullname = (
                    primary_parent.user.first_name + " " + primary_parent.user.last_name
                )
    ...
    def mutate(root, info, **validated_data):
        try:
            announcement_obj = Announcement.objects.get(
                id=validated_data.get("announcement_id")
            )
...
class MutateParentNotificationSettings(graphene.Mutation):
    class Arguments:
        ...
        missed_session_reminder_email = Boolean()
        missed_session_reminder_sms = Boolean()
    ...
    def mutate(root, info, parent_id, **validated_data):
        settings, created = ParentNotificationSettings.objects.update_or_create(
            parent_id=parent_id, defaults=validated_data
        )
...
class MutateInstructorNotificationSettings(graphene.Mutation):
    ...
    def mutate(root, info, instructor_id, **validated_data):
        settings, created = InstructorNotificationSettings.objects.update_or_create(
            instructor_id=instructor_id, defaults=validated_data
        )
...
class Mutation(graphene.ObjectType):
    ...
    create_instructor_notification_setting = (
        MutateInstructorNotificationSettings.Field()
    )
