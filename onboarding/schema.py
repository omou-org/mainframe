...
def create_account_templates(show_errors=False):
    ...
    parents_ws[
        "A1"
    ].value = "Instructions: Please enter parent details in this spreadsheet similar to the example on row 3. Double check that all emails, phone numbers, and zip codes are valid, otherwise those parents may not be uploaded correctly. The first row is an example that will not be uploaded."
    parents_ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=4)
    style_instruction_comment(parents_ws["A1"])

    parents_column_names = [
        "First Name",
        "Last Name",
        "Email",
        "Phone",
    ...
    instructors_ws[
        "A1"
    ].value = "Instructions: Please enter instructor details in this spreadsheet similar to the example on row 3. Instructors with invalid emails will not be entered into Omou, so please confirm the instructor's email is valid prior to submission! The first row is an example that will not be uploaded."
    instructors_ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=4)
    style_instruction_comment(instructors_ws["A1"])

    instructors_column_names = [
        "First Name",
        "Last Name",
        "Email",
        "Phone",
    ...
    instructors_example = [
        ...
        "vlane@gmail.com",
        "6453456765",
        "Vicky is experienced in teaching math for all skill levels.",
        "5",
        "456 Candy Lane",
        "Los Angeles",
        "CA",
...
def create_course_templates(business_id, show_errors=False):
    ...
    if total_instructors > 0:
        instructor_dv = DataValidation(
            ...
            formula1="{0}!$A$2:$A${1}".format(
                quote_sheetname("Instructor Roster (hidden)"), total_instructors
            ),
            allow_blank=False,
            showDropDown=False,
        )
    ...
    yes_no_dv = DataValidation(
        type="list", formula1='"Y,N"', allow_blank=False, showDropDown=False
    )
    ...
    tuition_dv = DataValidation(
        type="decimal", allow_blank=False, operator="greaterThan", formula1=0
    )
    course_ws.add_data_validation(tuition_dv)

    # capacity validation
    capacity_dv = DataValidation(
        type="whole", operator="greaterThanOrEqual", formula1=4
    )
    ...
    for c in range(len(class_column_names)):
        ...
        if class_column_names[c].startswith("Start Time") or class_column_names[
            c
        ].startswith("End Time"):
...
def create_enrollment_templates(business_id, show_errors=False):
    ...
    for course in active_and_future_courses:
        ...
        course_ws[
            "A1"
        ].value = "Instructions: This course enrollment sheet and course details are generated from saved courses in the Omou database. Row 9 and beyond are dropdowns of students existing in the Omou database. Please select a student from the dropdown list starting from row 9 to create an enrollment for this course."
        ...
        background_color = PatternFill(
            start_color="EBFAFF", end_color="EBFAFF", fill_type="solid"
        )
        ...
        if total_students > 0:
            student_dv = DataValidation(
                type="list",
                formula1="{0}!$A$2:$A${1}".format(
                    quote_sheetname("Student Roster (hidden)"), total_students
                ),
                allow_blank=False,
                showDropDown=False,
            )
