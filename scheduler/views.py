    ...
    def get(self, request, instructor_id):
        ...
        if not start_time or not end_time or not date:
            return Response(
                ...
                data={
                    "status": "state_time, end_time, and date are all required parameters"
                },
            )
    ...
    def get(self, request, instructor_id):
        ...
        if not start_time or not end_time or not start_date or not end_date:
            return Response(
                status=status.HTTP_400_BAD_REQUEST,
                data={
                    "status": "state_time, end_time, start_date, and "
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, pk=None):
        ...
        now = datetime.now()
        self.get_queryset().filter(course=course_id, end_datetime__gt=now).delete()
