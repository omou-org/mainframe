...
class CourseSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if course.start_date and course.end_date:
            ...
            while current_date <= end_date:
                start_datetime = datetime.combine(
                    current_date.date(), course.start_time
                )
                end_datetime = datetime.combine(current_date.date(), course.end_time)
                start_datetime = (
        ...
        if (
            ...
            and course.num_sessions
            and course.session_length
            and course.total_tuition
        ):
            course.hourly_tuition = course.total_tuition / (
                course.num_sessions * course.session_length
            )
        else:
    ...
    def update(self, instance, validated_data):
        now = datetime.now()
        sessions = Session.objects.filter(course=instance, start_datetime__gte=now)

        for session in sessions:
            pacific_tz = pytz.timezone("America/Los_Angeles")
            utc_start_datetime = session.start_datetime.replace(
                tzinfo=timezone.utc
            ).astimezone(tz=pacific_tz)
            utc_end_datetime = session.start_datetime.replace(
                tzinfo=timezone.utc
            ).astimezone(tz=pacific_tz)
            start_datetime = datetime.combine(
            ...
            session.start_datetime = pacific_tz.localize(start_datetime).astimezone(
                pytz.utc
            )
            session.end_datetime = pacific_tz.localize(end_datetime).astimezone(
                pytz.utc
            )
            session.save()
        ...
        if "end_date" in validated_data or validated_data.get("is_confirmed", False):
            ...
            if len(sessions) == 0 and validated_data.get("is_confirmed", False):
                ...
                current_date = arrow.get(latest_session.start_datetime.date()).shift(
                    weeks=+1
                )
