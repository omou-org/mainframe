...
class CourseType(DjangoObjectType):
    active_availability_list = List(
        CourseAvailabilityType, source="active_availability_list"
    )
...
class Query(object):
    ...
    course_availabilities = List(
        CourseAvailabilityType, course_id=ID(), availability_ids=List(ID)
    )
    course_categories = List(CourseCategoryType)
    course_notes = List(CourseNoteType, course_id=ID(required=True))
    enrollments = List(
        EnrollmentType, student_id=ID(), course_id=ID(), student_ids=List(ID)
    )
    ...
    popular_categories = List(
        PopularCategoryType, timeframe=LookbackTimeframe(required=True)
    )
    ...
    def resolve_num_recent_sessions(self, info, timeframe, **kwargs):
        ...
        if timeframe == LookbackTimeframe.YESTERDAY:
            ...
            return Session.objects.filter(start_datetime__lte=now.datetime).count()
    ...
    def resolve_popular_categories(self, info, timeframe, **kwargs):
        ...
        top_5_categories = sorted(
            category_counts, key=lambda item: item["num_sessions"], reverse=True
        )[:5]
